...
extern unsigned long monhi, monlo;
...
int range_ok(unsigned int u)
{
	...
	return(1);
    if (u >= ((unsigned long) STACK_BASE - stack_size_act) && 
	u < (unsigned long) STACK_BASE)
	return(1);
    return(0);
}
...
{
    char buf[200];
    if (addr >= (unsigned long) CODE_BASE 
	&& addr < ((unsigned long) (CODE_BASE + code_size)))
	return(code_seg[addr-CODE_BASE]);
    else if (addr >= ((unsigned long) (STACK_BASE - stack_size_act)) 
	     && addr <= (unsigned long) STACK_BASE)
	return(stack_seg[addr-(STACK_BASE-stack_size_act)]);
    else
    {
	sprintf(buf,"Illegal memory read: address [%lx] out of bounds\n", addr);
	run_error(buf);
    }
}

void write_byte(unsigned char b, unsigned long addr)
{
    char buf[200];
    if (monon)
    {
    ...
    {
	sprintf(buf,"Illegal memory write: address [%lx] out of bounds\n", 
		addr);
	run_error(buf);
    }
}

unsigned short read_half(unsigned long addr)
{
    char buf[200];
    unsigned short tmp;
    if (addr & 0x1)
    {
	sprintf(buf,"Illegal memory read: unaligned halfword address [%lx]\n",
		addr);
	run_error(buf);
    }
...
void write_half(unsigned short h, unsigned long addr)
{
    char buf[200];
    unsigned char b;
    if (monon)
...
unsigned long read_word(unsigned long addr)
{
    ...
    if (addr & 0x3)
    {
	sprintf(buf,"Illegal memory read: unaligned word address [%lx]\n",
		addr);
	run_error(buf);
    }
...
void write_word(unsigned long w, unsigned long addr)
{
    char buf[200];
    unsigned char b;
    if (monon)
...
double read_double(unsigned long addr)
{
    ...
    if (addr & 0x07)
    {
	sprintf(buf,"Illegal memory read: unaligned double address [%lx]\n",
		addr);
	run_error(buf);
    }
    if (little_endian)
...
void write_double(double d, unsigned long addr)
{
    char buf[200];
    unsigned long *p, tmp;
    ...
    if (addr & 0x07)
    {
	sprintf(buf,"Illegal memory write: unaligned double address [%lx]\n",
		addr);
	run_error(buf);
    }
    p = (unsigned long *) &d;
...
char *actual_address(unsigned long addr)
{
    ...
    {
	sprintf(buf,"Can't map memory address: address [%lx] out of bounds\n", 
		addr);
	run_error(buf);
    }
}
