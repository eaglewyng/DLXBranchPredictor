...
extern char *actual_address(unsigned long);
extern double read_double(unsigned long);
extern unsigned long read_word(unsigned long);
...
{
    unsigned long stackptr, res;
    unsigned long arg1, arg4;
    char *arg1ch;
    int size, nitems;
...
void do_printf(void)		/* trap #5 */
{
    ...
    int buf_index;
    unsigned long stackptr, fmt_index;
...
void do_fscanf(void)		/* trap #8 */
{
    ...
    */
    ...
    unsigned long stackptr, fmtstr, tmp;
    char *p;
    char buf[80];
	...
	if (*p == '%')
	{
		...
		{		/* get a long unsigned */
		    index++;
		    tmp = read_word(stackptr); stackptr += 4;
		    ulongp = (unsigned long *) actual_address(tmp);
		    buf[index] = EOS;
		...
		{		/* get a float */
		    ...
		    write_word(*((unsigned long *) &ftmp), tmp); 
...
void do_sprintf(void)		/* trap #11 */
{
    ...
    int buf_index;
    int stmp;
    unsigned long stackptr, fmt_index, str_index;
