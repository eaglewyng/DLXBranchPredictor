...
extern double read_double(unsigned long);
...
unsigned long monhi;		/* high address for monitor */
unsigned long monlo;		/* low address for monitor */
...
struct mapper *findmap(unsigned long id)
...
void out_instr(void)
{
    unsigned long rs1, rs2, rd, immed, immed26;
    long simmed, simmed26;
    char buf[100];
    struct mapper *mp;
    ...
    case FD2:
    case FREG2a:
	sprintf(buf,"%-8s f%ld,f%ld", mp->name, rd, rs1);
	break;
    case DREG2b:
    case FREG2b:
	sprintf(buf,"%-8s f%ld,f%ld", mp->name, rs1, rs2);
	break;
    case DREG3:
    case FREG3:
	sprintf(buf,"%-8s f%ld,f%ld,f%ld", mp->name, rd, rs1, rs2);
	break;
    case FI2:
	sprintf(buf,"%-8s f%ld,r%ld", mp->name, rd, rs1);
	break;
    case IF2:
	sprintf(buf,"%-8s r%ld,f%ld", mp->name, rd, rs1);
	break;
    case IMM1:
	sprintf(buf,"%-8s #%ld", mp->name, immed26);
	break;
    case IREG1:
	sprintf(buf,"%-8s r%ld", mp->name, rs1);
	break;
    case LEXP16:
	sprintf(buf,"%-8s %+ld", mp->name, simmed);
	break;
    case LEXP26:
	sprintf(buf,"%-8s %+ld", mp->name, simmed26);
	break;
    case LOADD:
    case LOADF:
	sprintf(buf,"%-8s f%ld,%+ld(r%ld)", mp->name, rs2, simmed, rs1);
	break;
    case LOADI:
	sprintf(buf,"%-8s r%ld,%+ld(r%ld)", mp->name, rs2, simmed, rs1);
	break;
    case NONEOP:
	sprintf(buf,"%-8s ", mp->name);
	break;
    case REG1IMM:
	sprintf(buf,"%-8s r%ld,%ld", mp->name, rs2, immed);
	break;
    case REG2IMM:
	sprintf(buf,"%-8s r%ld,r%ld,#%ld", mp->name, rs2, rs1, simmed);
	break;
    case REG3IMM:
	sprintf(buf,"%-8s r%ld,r%ld,r%ld", mp->name, rd, rs1, rs2);
	break;
    case REGLAB:
	sprintf(buf,"%-8s r%ld,%+ld", mp->name, rs1, simmed);
	break;
    case STRD:
    case STRF:
	sprintf(buf,"%-8s %+ld(r%ld),f%ld", mp->name, simmed, rs1, rs2);
	break;
    case STRI:
	sprintf(buf,"%-8s %+ld(r%ld),r%ld", mp->name, simmed, rs1, rs2);
	break;
    case UNIMP:
...
int myfread(unsigned long *x, FILE *fp)
{
...
int load_binary(char *fname)
{
	...
	printf("File needs at least %ld bytes (static), and memory is %d bytes.\n", 
    ...
    tmpp = (unsigned long *) &(code_seg[tmp]);
